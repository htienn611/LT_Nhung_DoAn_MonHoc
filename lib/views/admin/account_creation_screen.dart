            ...
            children: [
                ClipRRect(
                  ...
                  Padding(padding: EdgeInsets.all(5)),
                    TextField(
                    decoration: InputDecoration(
                      hintText: 'Email hoặc Số điện thoại',hintStyle: TextStyle(fontSize: 18),
                      border: OutlineInputBorder(borderRadius: BorderRadius.circular(20)),
                  ...
                  Row(
                    ...
                    children: [
                      Checkbox(value: _auth, onChanged: (bool? value ){
                        setState(() {
                          _auth=value!;
                        });
                      },),
                      Text("Quản lý tài khoản",style: TextStyle(color: Colors.blueGrey,fontSize: 18),)
                    ],
                  ),
                  ElevatedButton(onPressed: (){
                    //kt
                    if(_name.text.isEmpty||_unit.text.isEmpty||_pass.text.isEmpty||_repass.text.isEmpty)
                  {
                    return ;
                  }
                      ...
                      if(_pass.text==_repass.text)
                      {
                        checkData(_unit.text).then((value) => value==false?(
                         data(_name.text, _unit.text, _pass.text, _auth).then((value) => value.id==null?tb="Đăng kí tài khoản không thành công":tb="Đăng kí tài khoản thành công")                        ):tb="tài khoản đã tồn tại");
                      }
                      else
                      {
                 ...
                 Text(tb,style: TextStyle(color: Colors.red,fontSize: 18),),
            ],
          ),
        ),
      ),
